...
class VenueAnalyticsService {
     ...
     */
async getHotDatesAnalytics({ venueId, from, to, limit = 50 }) {
    console.log('🚀 [getHotDatesAnalytics] Initiated with params:', { venueId, from, to, limit });
    ...
    try {
        ...
        // FIXED: More flexible date parsing
        const pipeline = [
            { $match: { 
                ...matchStage, 
                'engagement.actions.startFilterDate': { 
                    $exists: true, 
                    $ne: null, 
                    $ne: "" 
                } 
                        ...
                        $switch: {
                            branches: [
                                // Try DD/MM/YYYY format
                                {
                                    case: {
                                        $regexMatch: {
                                            input: '$engagement.actions.startFilterDate',
                                            regex: /^\d{2}\/\d{2}\/\d{4}$/
                                        }
                                    },
                                    then: {
                                        $dateFromString: {
                                            dateString: '$engagement.actions.startFilterDate',
                                            format: "%d/%m/%Y",
                                            onError: null
                                        }
                                    }
        ...
        if (from || to) {
            ...
            pipeline.push({ $match: { eventDate: dateFilter } });
        }
        ...
        pipeline.push(
            ...
            { $sort: { totalInterest: -1, eventDate: 1 } },
            { $limit: parseInt(limit) }
        );

        console.log('📋 [getHotDatesAnalytics] Executing Pipeline:', JSON.stringify(pipeline, null, 2));

        const result = await collection.aggregate(pipeline).toArray();

        console.log('📦 [getHotDatesAnalytics] Raw result:', result);
        ...
        const finalResult = result.map(item => ({
            ...
            heatIntensity: Math.min(item.heatIntensity || 0, 1),
            occasions: (item.occasions || []).filter(o => o && o !== null),
            averageGuestCount: 0 // Will be calculated separately if needed
        }));

        console.log('✅ [getHotDatesAnalytics] Final result:', finalResult);
        return finalResult;

    } catch (error) {
        console.error('❌ [getHotDatesAnalytics] Error:', error);
        throw error;
    }
}
    /**
