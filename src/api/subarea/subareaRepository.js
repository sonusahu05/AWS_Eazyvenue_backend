...
class SubareaRepository extends BaseRepository {
  ...
  listFiltered(filter1) {    
    ...
    var cityNameSearch={ $match:{}};
    if (typeof filter1.filterByCityName !== 'undefined') {
      cityNameSearch = {
          $match:{"citydata.name":{ $regex: filter1.filterByCityName, $options: 'i' }},
        };
    }    
  ...
  _getListFilter(filter) {
    ...
    if(copyFilter.filterByCityId) {
      copyFilter.query.city_id = {'$eq': ObjectId(copyFilter.filterByCityId)};
    }
    return copyFilter;
  }
}
...
module.exports = SubareaRepository;
